...
## Model leaderboard & chosen checkpoint

Aggregate **`metrics.json`** across `runs/*/`, and pick minimise-FP-first with optional recall floor:

```bash
python compare_runs.py              # Markdown table sorted by macro-F1
python compare_runs.py --pick-best # winner per benchmark size ``n``
```

Interpretation + fixed thesis recommendation: **[MODEL_SELECTION.md](MODEL_SELECTION.md)**.

## Using this classifier in another repo

For inference-only integration (analysis project, same checkpoint, fixed threshold), see **[PORTING.md](PORTING.md)** — which files and the `runs/…` directory to copy, and a sample `predict.py` command with `--threshold 0.8`.
